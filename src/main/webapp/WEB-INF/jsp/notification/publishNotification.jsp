...
<body>
	<div id="main">
		<form method="post" class="form form-x form-auto form-tips" id="publishNotification" onsubmit="return publishNotification()">
			<div class="form-group">
				<div class="label">
					<label>公告标题</label>
				</div>
				<div class="field">
					<input class="input" size="110" type="text" name="title" data-validate="required:必填" placeholder="公告标题" />
				</div>
			</div>
			<div class="form-group">
				<div class="label">
				...
				<div class="field">
					<input class="input" size="110" type="text" name="key"  data-validate="required:必填" placeholder="关键字,多个关键字使用'|'分割" />
				</div>
			</div>
			<div class="form-group">
				<div class="label">
				...
				<div class="field">
					<input onclick="laydate()" class="input" size="110" type="text" name="createTime" value="<%= DateUtil.formatDate(new Date(),"yyyy-MM-dd")%>" data-validate="required:必填" placeholder="发布时间"/>
				</div>
			</div>
			<div class="form-group">
			   <div class="label"><label for="upfile">公告展示图片</label></div>
			    <div class="field">
			      <img alt="" id="upurl" width="" height="0px;" style="visibility:hidden;" src="">
		...
		<script type="text/javascript">
			...
			function publishNotification(){
				...
				if(title.length<=0||createTime.length<=0||content.length<=0){
					dialog({title:"操作失败",content:"请填写必填项", width:400}).show();
					return false;
				}
				var postData = "title="+title+"&key="+key+"&createTime="+createTime+"&images="+images+"&content="+content;
				$.ajax({url:"${pageContext.request.contextPath}/info/publishNotification?date="+new Date().getTime(),data : postData,type:"POST",success:function(data){
					if(data.success){
						var teacherOperatorDialog =dialog({title:"发布成功"
							,content:data.msg
							, width:400
							,okValue: '确定'
						    ,ok: function () {
