	...
	<div id="main">
		<form method="post" class="form form-x form-auto form-tips" id="publishNews" onsubmit="return publishNews()">
			...
			<div class="form-group">
				...
				<div class="field">
					<input class="input" size="110" type="text" value="本站" name="source" data-validate="required:必填"  />
				</div>
			</div>
			<div class="form-group">
				<div class="label">
				...
				<div class="field">
					<input class="input" size="110" type="text" name="key"  data-validate="required:必填"  />
				</div>
			</div>
			<div class="form-group">
				<div class="label">
				...
				<div class="field">
					<input onclick="laydate()" class="input" size="110" type="text" name="createTime" />
				</div>
			</div>
			<div class="form-group">
		...
		<script type="text/javascript">
			...
			function publishNews(){
				...
				if(title.length<=0||source.length<=0||createTime.length<=0||content.length<=0){
					dialog({title:"操作失败",content:"请填写必填项", width:400}).show();
					return false;
				}
				var postData = "title="+title+"&source="+source+"&key="+key+"&createTime="+createTime+"&images="+images+"&content="+content;
				$.ajax({url:"${pageContext.request.contextPath}/info/publishNews?date="+new Date().getTime(),data : postData,type:"POST",success:function(data){
					if(data.success){
