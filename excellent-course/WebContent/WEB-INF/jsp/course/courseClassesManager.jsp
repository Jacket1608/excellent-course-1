...
<table class="table">
	...
	<c:forEach items="${pager.datas }" var="item">
		<tr>
			<td>${item.title }</td>
			<td>
				<div style="width: 400px; height: 40px;; overflow: hidden;">${item.description }</div>
			</td>
			<td><fmt:formatDate value="${item.craeteTime }" pattern="yyyy-MM-dd" /></td>
			<td>
				<div style="width: 100px;">
					<a href="javascript:editCourseClasss('${item.id }')" class="button button-small bg-sub">编辑</a>
					<a href="${pageContext.request.contextPath }/course/view/${item.id }/courseClass"
