	...
	<div id="main">
		<form method="post" class="form form-x  form-tips" id="publishNews" onsubmit="return publishNews()">
			<div class="form-group">
				...
				<div class="field">
					<input class="input" size="110" value="${classDetail.id }" type="hidden" name="id" />
					<input class="input" size="110" value="${classDetail.title }" type="text" name="title" data-validate="required:必填" />
				</div>
			</div>
			<div class="form-group">
				<div class="label">
				...
				<div class="field">
					<select class="input" name="courseClass">
						...
						<c:forEach var="item" items="${courseClasses }">
							<option value="${item.id }" ${item.id==classDetail.courseClass.id?"selected":"" }>${item.title }</option>
						</c:forEach>
						</select>
				</div>
			</div>
		<!-- 	<div class="form-group">
			   ...
			   <div class="label"><label for="upfile">附件</label></div>
			    ...
			    <c:if test="${!empty attachs }">
			    	...
			    	<c:forEach var="item" items="${ attachs}">
			    		...
			    		<label>${item.fileName }</label><button onclick="deleteItem(this)">删除</button>
			    	</div>
			    	</c:forEach>
			    </c:if>
			    </div>
			...
			<div class="form-group">
				...
				<div class="field">
					<textarea name="" id="content" style="width: 750px; height: 500px;">${classDetail.content }</textarea>
				</div>
			</div>
		...
		<script type="text/javascript">
			...
			function publishNews(){
				...
				$.ajax({url:"${pageContext.request.contextPath}/course/editClassDetail/"+id+"?date="+new Date().getTime(),data : postData,type:"POST",success:function(data){
					if(data.success){
